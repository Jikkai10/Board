...
public class BoardQueryService {
    ...
    public List<List<MovementsCardEntity>> getMovementsCardsFinal(final Long id) throws SQLException {
        var dao = new BoardDAO(connection);
        var boardColumnDAO = new BoardColumnDAO(connection);
        var optional = dao.findById(id);
        if (optional.isPresent()) {
            var entity = optional.get();
            var columns = boardColumnDAO.findByBoardId(entity.getId());
            var service = new BoardColumnQueryService(connection);
            List<List<MovementsCardEntity>> movements = new ArrayList<>();
            for (var column : columns) {
                if (column.getKind() == FINAL) {
                    movements = service.getMovementsCards(column);
                }
            }
            return movements;
        }
